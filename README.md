...
- `sensor.household_tasks_unclaimed` / `_christine` / `_eduard` /
...
- Two services: `household_tasks.add_task` (name, assignee, recurring,
  ...
  structured input instead of typing `Assigned: Christine` into a text
...
as plain text in the item's description:
